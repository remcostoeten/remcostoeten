...
export function makeBreadcrumbs(crumbs: CrumbCopy, page: PageKey): HTMLElement {
  const nav = make("nav");
...
export function makeFooter(label: string): HTMLElement {
...
function makeNav(nav: NavCopy, page: PageKey): HTMLElement {
  const wrap = make("nav");
  wrap.className = "nav";
...
function makeNavLink(label: string, href: string, active: boolean): HTMLAnchorElement {
...
function makeActions(lang: LangCopy, locale: Locale, onLang: LangChange): HTMLElement {
  const wrap = make("div");
...
function makeCrumbLink(label: string, href: string): HTMLAnchorElement {
  const link = document.createElement("a");
  link.href = href;
  link.textContent = label;
  return link;
